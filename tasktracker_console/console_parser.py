...
def init_task_parser(root):
    task_parser = root.add_parser(Parser.TASK, help='Menu to work with tasks')
    task_root_subparser = task_parser.add_subparsers(dest=Parser.TASK, help='Choose action to perform')

    init_plan_parser(task_root_subparser)

    add_task_parser = task_root_subparser.add_parser(Parser.ADD, help='Add new task')
    add_task_parser.add_argument(Parser.prefix().TASK_PID,
                                 type=int, help='Add task in project with pid')
    add_task_parser.add_argument(Parser.prefix().TASK_TITLE,
                                 type=str, required=True, help='Type title of new task')
    add_task_parser.add_argument(Parser.prefix().TASK_DESCRIPTION,
                                 type=str, help='Type description of new task')
    add_task_parser.add_argument(Parser.prefix().TASK_START_TIME,
                                 type=get_time_arg(), 
                                 nargs='+', action=time_collect_action(),
                                 help=('Start time of task in format like [day]-[month]-[year] '
                                    'or line today[+-][shift]. Shift measured in days'))
    add_task_parser.add_argument(Parser.prefix().TASK_END_TIME,
                                 type=get_time_arg(), 
                                 nargs='+', action=time_collect_action(),
                                 help=('End time of task in format like [day]-[month]-[year] '
                                    'or line today[+-][shift]. Shift measured in days'))
    add_task_parser.add_argument(Parser.prefix().TASK_DEADLINE,
                                 type=get_time_arg(), 
                                 nargs='+', action=time_collect_action(),
                                 help=('Deadline time of task in format like [day]-[month]-[year] '
                                    'or line today[+-][shift]. Shift measured in days'))
    add_task_parser.add_argument(Parser.prefix().TASK_PARENT_ID,
                                 type=int, help='Add task as subtask of specified parent')
    ...
    show_task_parser = task_root_subparser.add_parser(Parser.SHOW, help='Show tasks')
    ...
    delete_task_parser = task_root_subparser.add_parser(Parser.DELETE, help='Delete tasks')
    delete_task_parser.add_argument(Parser.prefix().TASK_TID,
                                    type=int, required=True)

    edit_task_parser = task_root_subparser.add_parser(Parser.EDIT, help='Edit tasks')
...
def init_plan_parser(root):
    plan_parser = root.add_parser(Parser.PLAN, help='Menu to work with plans')
    plan_root_subparser = plan_parser.add_subparsers(dest=Parser.PLAN)

    add_plan_parser = plan_root_subparser.add_parser(Parser.ADD, help='Add new plan')
    ...
    delete_plan_parser = plan_root_subparser.add_parser(Parser.DELETE, help='Delete plan')
    delete_plan_parser.add_argument(Parser.prefix().PLAN_ID,
                                    type=int, required=True)

    show_plan_parser = plan_root_subparser.add_parser(Parser.SHOW, help='Show plans')
    ...
    edit_plan_parser = plan_root_subparser.add_parser(Parser.EDIT, help='Edit plan')
...
def init_project_parser(root):
    project_parser = root.add_parser(Parser.PROJECT, help='Menu to work with projects')
    project_root_subparser = project_parser.add_subparsers(dest=Parser.PROJECT)

    add_project_parser = project_root_subparser.add_parser(Parser.ADD, help='Add new project')
    add_project_parser.add_argument(Parser.prefix().PROJECT_NAME, 
                                    type=str, required=True)

    show_project_parser = project_root_subparser.add_parser(Parser.SHOW, help='Show projects')
    ...
    delete_project_parser = project_root_subparser.add_parser(Parser.DELETE, help='Delete project')
    delete_project_parser.add_argument(Parser.prefix().PROJECT_ID, 
                                     type=int, required=True)
    
    edit_project_parser = project_root_subparser.add_parser(Parser.EDIT, help='Edit project')
    edit_project_parser.add_argument(Parser.prefix().PROJECT_ID, 
                                     type=int, required=True)
    edit_project_parser.add_argument(Parser.prefix().PROJECT_NAME,
                                     type=str)

    invite_project_parser = project_root_subparser.add_parser(Parser.PROJECT_INVITE, help='Invite a user to project')
    ...
    exclude_project_parser = project_root_subparser.add_parser(Parser.PROJECT_EXCLUDE, help='Exclude a user from projects')
...
def init_user_parser(root):
    user_parser = root.add_parser(Parser.USER, help='Menu to work with users')
    user_root_subparser = user_parser.add_subparsers(dest=Parser.USER)

    add_user_parser = user_root_subparser.add_parser(Parser.ADD, help='Add new user')
    add_user_parser.add_argument(Parser.prefix().USER_LOGIN,
                                 type=str, required=True)

    show_user_parser = user_root_subparser.add_parser(Parser.SHOW, help='Show users')
    ...
    delete_user_parser = user_root_subparser.add_parser(Parser.DELETE, help='Delete user')
    delete_user_parser.add_argument(Parser.prefix().USER_UID)

    edit_user_parser = user_root_subparser.add_parser(Parser.EDIT, help='Edit user')
...
def init_login_parser(root):
    login_parser = root.add_parser(Parser.LOGIN, help='Login action')
